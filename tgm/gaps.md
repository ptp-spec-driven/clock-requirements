...
| :--- | :--- | :--- | :--- | :--- |
| G1 | HIGH | FUNC-TGM008-R7 | The system does not differentiate holdover-in-spec from holdover-out-of-spec. When holdover goes out of spec, it is reported as FREERUN. We need a metric or event that differentiates holdover-in-spec from holdover-out-of-spec. | linuxptp-daemon |
| G2 | HIGH | FUNC-TGM003-R2/R3 (Antenna Survey) | The system issues a GNSS survey-in command but does not wait for completion or persist the fixed 3D position after completion. | linuxptp-daemon |
...
| G4 | HIGH | FUNC-TGM006-R4 (System clock sync startup) | System clock synchronization is delayed but gated on `abs(offset) < 1s` from the reporting process, rather than explicitly waiting for the PHC to be fully locked. | linuxptp-daemon |
...
| G10 | MEDIUM | §13.1.2, FUNC-TGM003-R4 (GNSS config) | `antennaCableDelay` is not a user-configurable GNSS parameter. `GNSSInit` exposes only antenna voltage, constellations, survey-in and extra commands. | ptp-operator, linuxptp-daemon |
...
| :--- | :--- | :--- | :--- | :--- |
| G1 | `pkg/dpll/dpll.go` | 678-696 | `DPLL_HOLDOVER` out-of-spec branch sets `state = PTP_FREERUN` with an explicit `// TODO: GNSS holdover currently doesn't offer holdover out of spec` note. | Emit a metric or event that differentiates holdover-in-spec from holdover-out-of-spec, rather than simply reporting FREERUN. |
| G1 | `pkg/event/event.go` | 445-451 | Composite `updateGMState` reports `state=FREERUN` and `clockClass=140` when `outOfSpec && frequencyTraceable`. | Emit a metric or event that differentiates holdover-in-spec from holdover-out-of-spec. |
| G2 | `pkg/hardwareconfig/gnss.go`, `pkg/ublox/ublox.go` | `gnss.go:65-77,106-127`, `ublox.go:157` | Issues `ubxtool -e SURVEYIN` with a 5s wait. | Wait for survey completion (hours) and persist the fixed 3D position to NVM after a fixed position is acquired. |
...
| G4 | `pkg/daemon/daemon.go` | 2146-2154 | System clock synchronization is enabled when `math.Abs(offset) < 1000000000` (sub-second). | Ensure system clock synchronization does not begin until the PHC is fully locked (using an appropriate windowed average or offset measurement to determine lock status). |
...
| G10 | `pkg/hardwareconfig/gnss.go` | 106-127 | `buildGNSSInitCommands` emits antenna-voltage, constellation, survey-in and extra commands only. No cable-delay command. | Emit receiver config for `antennaCableDelay`. |
...
| G10 | `api/v2alpha1/hardwareconfig_types.go` | 167-207 | `GNSSInit` has `AntennaVoltage`, `Constellations`, `SurveyIn`, `ExtraCommands` only. | Add `antennaCableDelay` field (or document the delay-compensation-model equivalent for cable delay). |
...
1. **G1 (Holdover-Out-Of-Spec state)**: Implement a metric or event that differentiates holdover-in-spec from holdover-out-of-spec.
...
3. **G4 (System clock sync gating)**: Ensure system clock synchronization does not begin until the PHC is fully locked.
...
9. **G10 (GNSS cable delay)**: Add `antennaCableDelay` and emit the receiver configuration.
